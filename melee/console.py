...
class Console:
    ...
    def __handle_slippstream_menu_event(self, event_bytes, gamestate: GameState):
        ...
        # CSS costume indices from Extract Menu Info gecko payload.
        # Online (SLIPPI_ONLINE_CSS): upstream libmelee only ever reported P1's byte
        # at 0x3F (Melee RAM 0x803F0E09). Offline Versus CSS (CHARACTER_SELECT) uses
        # per-port payload offsets 0x3F / 0x49-0x4B once the fork gecko extension runs.
        try:
            if gamestate.menu_state == enums.Menu.SLIPPI_ONLINE_CSS:
                gamestate.players[1].costume = np.ndarray((1,), ">B", event_bytes, 0x3F)[0]
